...
class CGDataConvert extends BaseNode {
  ...
  getOutput(index) {
    ...
    if (curType === 'Double' || curType === 'Int') {
      this.outputs[0] = new APJS.Vector2f(value, value);
      this.outputs[1] = new APJS.Vector3f(value, value, value);
      this.outputs[2] = new APJS.Vector4f(value, value, value, value);
      this.outputs[3] = new APJS.Color(value, value, value, value);
      this.outputs[4] = `${Number.isInteger(value) ? value : value.toFixed(2)}`;
    } else if (curType === 'Vector2f') {
      this.outputs[0] = new APJS.Vector3f(value.x, value.y, 0.0);
      this.outputs[1] = new APJS.Vector4f(value.x, value.y, 0.0, 0.0);
      this.outputs[2] = `x:${value.x.toFixed(2)}, y:${value.y.toFixed(2)}`;
    } else if (curType === 'Vector3f') {
      this.outputs[0] = new APJS.Vector2f(value.x, value.y);
      this.outputs[1] = new APJS.Vector4f(value.x, value.y, value.z, 0.0);
      this.outputs[2] = new APJS.Color(value.x, value.y, value.z, 0.0);
      this.outputs[3] = `x:${value.x.toFixed(2)}, y:${value.y.toFixed(2)}, z:${value.z.toFixed(2)}`;
    } else if (curType === 'Vector4f') {
      this.outputs[0] = new APJS.Vector2f(value.x, value.y);
      this.outputs[1] = new APJS.Vector3f(value.x, value.y, value.z);
      this.outputs[2] = new APJS.Color(value.x, value.y, value.z, value.w);
      this.outputs[3] = `x:${value.x.toFixed(2)}, y:${value.y.toFixed(2)}, z:${value.z.toFixed(2)}, w:${value.w.toFixed(
        2
      )}`;
      this.outputs[4] = new APJS.Rect(value.x, value.y, value.z, value.w);
    } else if (curType === 'Color') {
      this.outputs[0] = new APJS.Vector3f(value.r, value.g, value.b);
      this.outputs[1] = new APJS.Vector4f(value.r, value.g, value.b, value.a);
      ...
      this.outputs[0] = new APJS.Vector4f(value.x, value.y, value.width, value.height);
    ...
    //   this.outputs[0] = new APJS.Vector4f(value.x, value.y, value.z, value.w);
